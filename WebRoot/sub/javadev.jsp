...
<html>
  <head>
    ...
    <title>My JSP 'embedhardware.jsp' starting page</title>
  ...
  <body>
    <form action="demoten.jsp" method="post">
		<div class="header">
			<h1>敦穆教育公众号管理系统</h1>
			<ul class="ulone">
				<li class="list lione">
					...
					<div class="lishreete">
						<ul class="listul">
							...
							<button type="button" name="changeBtn" id="changeBtn"
								class="xiubtn"
								onClick="window.open('demotenChange.jsp?id=<%=qt.getId()%>','_self')">修改</button>
							...
							<button type="button" name="removeBtn" id="removeBtn"
								class="shanbtn" onClick="window.open('demotenDel_done.jsp?id=<%=qt.getId() %>','_self')">删除</button>
							<%
								}
							%>
						</ul>
	...
	<!-- <script src="<%=basePath%>js/paging.min.js"></script> -->
